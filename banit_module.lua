...
local shadowBanMessages = {
	...
	"Invalid remote key generation.", "Remote key invalid.", "Core system keys are invalid."
}
...
Players.PlayerAdded:Connect(function(plr)
	...
	elseif timedBans[tostring(plr.UserId)] ~= nil and table.find(unbannables, plr.UserId) == nil then
		...
		local tonum = tonumber(banData[1])
		local timeLeft = os.time() - tonum
...
function BanIt.kick(username, reason, delay)
